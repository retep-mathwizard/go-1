...
func main() {
	connection := comm.Start("1234")
	rand.Seed(time.Now().UTC().UnixNano())
	xturn := rand.Float32() < 0.5
	who := "o"
	if xturn {
		who = "x"
	}
	...
	var position int
	var err error
	fmt.Println(who, "goes first (you're x)")
	time.Sleep(5 * time.Second)
	for xwon != true && owon != true && tie != true {
		fmt.Println("\033[H\033[2J")
		fmt.Println("You are x, your opponent is o. Pick a number from 1-9 starting at the top left and ending at the bottom right.")
		printboard()
		if !xturn {
